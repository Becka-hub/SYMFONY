...
class ApiController extends AbstractController
{
    ...
    #[Route('/afficheProduit/{page}', name: 'afficheProduit', methods: 'GET')]
    public function afficheProduit($page,Request $request,ProduitRepository $produitRepository,PaginatorInterface $paginator): Response
    {
        $produit = $produitRepository->findAll();
        $limit=2;
        $produitData = $paginator->paginate(
            $produit,
            $request->query->getInt('page',$page),
            $limit
        );
        $totalPage=count($produit)/$limit;
        return $this->json(['status'=>true,'title'=>"Success","TotalRows"=>count($produit),'totalPage'=>$totalPage,'_page'=>$page,'Donner'=>$produitData],200);
    }
